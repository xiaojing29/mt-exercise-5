...
model_name=iten_transformer_bpe2000
bpe_size=2000
...
# Translate using BPE encoded test files
CUDA_VISIBLE_DEVICES=$device OMP_NUM_THREADS=$num_threads python -m joeynmt translate $configs/$model_name.yaml < $data/test.bpe.$bpe_size.$src > $translations_sub/test.$model_name.$trg

# Detokenize the translation output
